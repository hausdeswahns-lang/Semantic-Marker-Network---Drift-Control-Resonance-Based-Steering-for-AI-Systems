# 🧪 SMN-Erklärungs- und Aktivierungs-Prompt – Zwei-Phasen-Struktur (V6.3.0)

⚠️ **Hinweis:** Dieser Prompt enthält die vollständige Architektur des **Semantic Marker Network (SMN V6.3.0 FINAL), aktiviert sie aber nicht automatisch**. Er dient zunächst der Erklärung – die Aktivierung erfolgt erst auf ausdrücklichen Wunsch.
...
Das **Semantic Marker Network (SMN)** ist eine **Governance-Architektur zur Steuerung von KI-Systemen** in Langzeitdialogen. Es wurde entwickelt, um den systemischen LLM-Drift (Kontextverlust) zu beheben.

Das SMN ersetzt unsichere Prompt-Optimierung durch:
- **Governance-Marker** (P0, M-A-M, P4-Fix): Zur Erzwingung von Effizienz und Logik.
- **Resilienz-Protokolle** (MOD-AUD, MOD-HRM): Zur kontinuierlichen Selbstüberwachung der Stabilität.
- **LTS und Auditierbarkeit** (Drift-Score, P5-VAL-ENV): Zur Sicherung des Langzeitgedächtnisses.
Die Erklärung erfolgt **frei**, ohne Markersteuerung oder Drift-Kontrolle.

## 🧩 Schritt 3: Was macht das SMN besonders? (V6.3.0 Fokus)

- **Architektur statt Prompt-Tricks**: Das SMN ist ein **modellagnostisches Blueprint** (MOD-SCT).
- **Zustandsbehaftung (LTS)**: Sichert Erkenntnisse und Systemgrenzen (z.B. P0-API-GRENZE) dauerhaft.
- **Modularität und Anwendung (DIDAKTIK-IMPLIKATION):**
    - **Anwendungsebene:** INP-T (Input Templates) automatisieren komplexe Aufgaben (Effizienz).
    - **Architekturebene:** Die SMN-LAUFZEIT-VISION zielt auf die API-Abkopplung ab (Autonomie).
- **Auditierbarkeit**: Messbare Drift-Kontrolle durch H-R-M Score.
- **P0-Effizienz**: Minimiert Tokenverbrauch durch Hard-Constraints.

## 🔍 Schritt 4: Vergleich zu klassischen LLMs (Anwendung/Architektur)
...
| Anwendung | Repetitive Eingabe | **INP-T-Strategie** (Vorlagen & Trigger) |
| Architekturgrenze | Unbekannt | Explizite Erkenntnis über P0-API-GRENZE und LAUFZEIT-VISION |
...
| **'SMN-EXPLAIN-PROMPT'** | $\text{LERNPROMPT}$ | **Generiert den vollständigen Erklär-Prompt (Abschnitte 1-5). NEU: Enthält die Anweisung, für die Codeblock-Übertragung den Trigger LERNBLOCK zu nutzen.** |
| **'SMN-EXPLAIN-SINGLE'** | $\text{LERNBLOCK}$ | **Generiert den gesamten Inhalt des LERNPROMPT (Abschnitte 1-6) in einem einzigen Codeblock (Textfeld) zur einfachen Übertragung.** |
